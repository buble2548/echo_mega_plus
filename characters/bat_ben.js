...
//
//  หมายเหตุ: กลไก 3 อย่างของตัวนี้เกาะกับ shared infra ของ server.js จึงมี call site อยู่ที่นั่นด้วย
//    1) เร้นเงา (batStealth) สลายทันทีเมื่อเสียเลือด/เกราะ -> onDamaged() ถูกเรียกจาก loseHp()/loseArmor()
//       (จุดเดียวกับ CHAR_HOOKS.phenex.onHpLost/onArmorLost)
//    2) เร้นเงาหมดเวลาเอง -> onStealthExpire() ถูกเรียกจากลูปลดเทิร์นสถานะใน endTurn()
//       (แพทเทิร์นเดียวกับ "wither" ของเรียวกิ ชิกิ)
//    3) กรรมถึงตัว (batKarma) รอเลือกเป้าหมายส่งต่อความเสียหาย -> p.batKarmaAsk + socket handler
...
const BAT_STEALTH_HEAL = 1;           // เร้นเงา: ไม่โดนความเสียหาย -> ฟื้นพลังชีวิต +1 ต่อเทิร์น
...
const BAT_KARMA_TURNS = 2;            // กรรมถึงตัว: คงอยู่ 2 เทิร์น (ทำงานได้ 1 ครั้งแล้วหายไป)
...
module.exports = {
  ...
  onRoundStartTick(engine, p) {
    ...
    // เร้นเงา: เทิร์นที่ผ่านมาไม่โดนความเสียหายเลย (ถ้าโดนสถานะจะสลายไปแล้ว) -> ฟื้นพลังชีวิต +1
    if ((p.statuses.batStealth || 0) > 0) {
      const heal = engine.healHp(p, BAT_STEALTH_HEAL);
      engine.log(`🌑 ${p.name} เร้นเงา — ไม่มีใครแตะต้องได้ ฟื้นพลังชีวิต +${heal} (เหลืออีก ${p.statuses.batStealth} เทิร์น · โจมตีไม่ได้ระหว่างนี้)`);
    }
  ...
  activateStealth(engine, p) {
    ...
    engine.log(`🌑 ${p.name} เร้นเงา — หายเข้าไปในความมืด ${BAT_STEALTH_TURNS} เทิร์น! ${got ? `ได้รับหลบหลีก +1 · ` : `(หลบหลีกเต็มเพดานแล้ว) · `}ไม่โดนความเสียหายจะฟื้นพลังชีวิต +${BAT_STEALTH_HEAL} ต่อเทิร์น — โจมตีไม่ได้ระหว่างนี้ และหากโดนความเสียหาย เร้นเงาจะสลายทันที`);
  },

  // เรียกจาก loseHp()/loseArmor() — โดนความเสียหายเมื่อไหร่ เร้นเงาสลายทันที (ไม่ระเบิด ไม่ใบ้สกิลใคร)
  onDamaged(engine, p) {
    if (!p || p.characterId !== "bat_ben" || !((p.statuses.batStealth || 0) > 0)) return;
    delete p.statuses.batStealth;
    if (p.statusAmt) delete p.statusAmt.batStealth;
    engine.log(`🌑💥 ${p.name} เร้นเงาสลาย — โดนความเสียหายเข้าจนหลุดออกจากเงามืด (กับดักไม่ทำงาน)`);
  },

  // เรียกจากลูปลดเทิร์นสถานะใน endTurn() ตอน batStealth หมดเวลาเอง
